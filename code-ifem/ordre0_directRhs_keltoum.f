...
c
c      s = 5
c	  do nb = (s-1)*nbtet + 1, s*nbtet
c	   nm1 = tetra(1, nb)
c 	    do j = 2, 4
c	     nm2 = tetra(j, nb)
c		 do i = 1, 3
c	      bk(i, j-1) = xin(i,nm2) - xin(i, nm1)
c		 enddo
c	    enddo
c	    absdete = det(Bk)
c	    absdete  = dabs(absdete)
c        do k = 1, 4
c	     do j = k, 4
c	      l = ((j-1)*j)/2 + k
c	      ww0(l, nb) = w0ref(k,j)*absdete
c	     enddo
c        enddo
c        enddo
	   return
          ...
          do i1 = 1, ni
           ...
           do i = 1, nq
            ...
            do k = kk, 4
             ...
             m = tetra(k, nb)
             if (m.ne.1) v_rhs(m-1) = v_rhs(m-1) + prd*res0*valrhs
	       enddo
	      enddo
         enddo
...
c	   call triangles(1, mu_n, absdet_bis, nmu)

c       call dec_gauss_lobatto(0.D0, 1.D0, Ng, rwgau, rdgau,
c     &                         wgau, dgau)
...
c	     nm2 = tetra(j, nb)
c		 do i = 1, 3
c	      bk(i, j-1) = xin(i,nm2) - VTX0(i)
c		 enddo
c	    enddo
c	    absdete = det(Bk)
c	    absdete  = dabs(absdete)

c        call inversion(BK, IBK)
c	    call transpos(3, 3, IBK, IBK_T)

c	    call matv3(IBK, VTX0, vc0)

c	     do k = 1, 4
c 	       call nmat_v3(k, IBK_T, gradref, va)
c           res = rval0(k)
c	       do m = 1, 3
c	        res = res - gradref(m, k)*vc0(m)
c	       enddo
c           val0(k) = res
c	     enddo
...
c	     do ss = 1, Ng
c          delt = dgau(ss)
c	      prd1 = wgau(ss)*absdete*delt*delt
c          do i = 1, nq
c		    prd = weiq(i)*absdet_bis*prd1
c		    do m = 1, 3
c              xp(m) = delt*nmu(m, i)
c            enddo

c            call matv3(BK, xp, vxp)
c            vxp = vxp + VTX0
c		    call ex_function(exmp, iso, vxp, valu, valrhs)

c            do k = 1, 4
c             res = rval0(k)
c	         do m = 1, 3
c	          res = res + gradref(m, k)*xp(m)
c	         enddo
!             call prodcolumns(i, k, nmu, va, res)
!             res0 = val0(k) + res*delt
c             m = tetra(k, nb)
c             v_rhs(m-1) = v_rhs(m-1) + prd*res*valrhs
c	        enddo
c	      enddo
c         enddo
