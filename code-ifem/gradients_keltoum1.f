...
c            if (iso.eq.1) then
c             call average_Asca_elem(exmp,  BK,  VTX0,
c     &                              avrvalue_a)
c             do k = 1, 4
c	          do j = k, 4
c               call prodcolumns(k, j, va, va, res)
c	           l = ((j-1)*j)/2 + k
c	           grad_grad(l, nb) = res*absdete*avrvalue_a/6.D0
c	          enddo
c             enddo
c            else
c             call average_Amat_elem(exmp, BK, verct,
c     &                              AvrgMat)
...
c              do m = 1, 3
c               do n = 1, 3
c                res =  res + va(n,k)*AvrgMat(n,m)*va(m,j)
c               enddo
c              enddo
c	          l = ((j-1)*j)/2 + k
c	          grad_grad(l, nb) = res*absdete/6.D0
c	         enddo
c            enddo
c            endif
