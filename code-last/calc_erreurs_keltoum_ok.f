...
c-----------------------------------------------------------
c      Etat de modification : fini
      subroutine prodsc_L2_grad(u, v, prod, prodi)
	  implicit none
      include 'fondements_keltoum.h'
	  include 'constants_keltoum.h'
!       this subroutine computes the scalar product in L2
...
c       Etat d'avancement: fini (aucun changement)
        subroutine norm_L2(gamma, u, nrm2, nrm2i)
!       this subroutine computes the result
!        norm_L2( u )= (int(u*u)dx)^(1/2)
	    implicit none
	    double precision u(*), nrm2, res, gamma, nrm2i

        call prodsc_L2(gamma, gamma,  u, u, res, nrm2i)
	    nrm2 = dsqrt(res)
  	    nrm2i = dsqrt(nrm2i)
        return
	    end
c-------------------------------------------------------------
c       Etat d'avancement: fini (aucun changement)
        subroutine norm_grad_L2(u, nrm2, nrm2i)
!       this subroutine computes the result
!        norm_L2(grad_u)= (int(grad_u*grad_u))^(1/2)
	    implicit none
	    double precision u(*), nrm2, res, nrm2i

        call prodsc_L2_grad(u, u, res, nrm2i)
	    nrm2 = dsqrt(res)
	    nrm2i = dsqrt(nrm2i)
        return
	    end
...
!       computing  norm_L2(u - v)
     ...
     &                     erreuri
        ...
        call norm_L2(gamma, w, erreur, erreuri)
	    return
        end
c-------------------------------------------------------------
c       Etat d'avancement: fini (aucun changement)
...
!       comuting norm_L2(grad_u - grad_v)
     ...
     &           erreur, gamma, erreuri
        ...
        call norm_grad_L2(w, erreur, erreuri)
	    return
        end
c------------------------------------------------------------
c      Etat d'avancement: fini (aucun changement)
     ...
     &    errGrdi)
        call norm_grad_L2(u,  nrm1, nrm1i)



       call norm_L2(gammau1, u1,  nrm0, nrm0i)
