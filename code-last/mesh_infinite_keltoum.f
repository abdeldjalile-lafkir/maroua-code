...
c       This  subroutine generates the mesh of the whole space after
c       sharing it into the union of one big tetrahedra K (the FEM region) and
c       4 infinite  simplices T1, T2, T3 and T4.
c       The finite element region K  is obtained from a fixed reference
c       tetrahedra RK using a homothety centered at the origin and
c       of ratio "pform". Thus, the parameter "pform" characterizes the size
c       of the finite element region. It characterizes also the distance of the
c       frontier between the FEM region and the IFEM region to the origin.
c
c       INPUT PARAMETERS
c        N           : This is an integer. This parameter N and the parameter
c                      mu suffice to generate the "mesh" in both the FEM region and  the virtual
c                      domain (obtained after inverting the infinite simplices)
c                      The greater is N the greater is the number of
c                      tetrahedra
c                      More precisely, N denotes the number of transverse levels i
c                      in the  mesh of one big tetrahedra
c        mu         : This is  the graduation parameter which characterizes the
c                       graduation of the mesh near to origin.
c                       mu must be between 0 and 1.
c                       It could be better to choose  mu greater than 0.5
c                       when mu = 1. the mesh is the virtual region is not graded

c        pform       :  is a real characterizing the  size
c                      of the finite element region. It characterizes also the distance of the
c                      bounbdary between the FEM regions and the IFEM region to the origin.
c                      The user can chose pform = 1 or 2.5 for example.
c      OUTPUT PARAMETERS
c        xin(3, *)   : an array containing the coordinates of the nodes
c        tetra(4, *) : an array containing the vertices numbers for all the tetrahedra
c                      tetra(i, k) returns the global vertex number of the i-th vertex
c                      in the k-th tetrahedra
c        domnod      : an array. for each vertex i, domnod(i) denotes the  number of the domain
c                      to which the vertex belongs. Domnod(i) is an integer
c                      between 1 and 5 (five domains)
c        domtet      : an array. for each tetrahera i, domtet(i) denotes the number of the domain
c                      in which the tetrahera belongs. Domtet(i) is an integer
c                      between 1 and 5 (five domains)


c       THE SAME IN FRENCH
c       Cette subroutine va generer le maillage du demi-espace apres
c       sa decomposition en quatre simplexes infinis et un simplexe fini.
c       Parametres d'entree
c        N           : Un entier qui indique le  nombre des niveaux
c                      du maillage dans chaque simplexe obtenu par inversion.
c        mu          : un parametre reel pour graduer le maillage pr s de l'origine.
c        pform       : un parametre de forme du simplexe element fini
c                      on peut la valeur prendre 1.
c       Parametres de sortie
c        tetra(4, *) : un tableau  contenant les numeros des  4 sommets de chacun
c                      des  tetraedres.
c        domnod      : un tableau qui indique pour chaque noeud le numero
c                      d'un des sous-domaines (grand tetra) auquels il appartient.
c        domtet      : un tableau qui indique pour chaque tetraedre le numero
c                      du sous-domaine (grand tetra) auquel il appartient.
c-----
       implicit none
       integer N
	   double precision mu, pform

	   integer nrep, i,j,m,k, nbnodesTet, dom
	   double precision xin(3, *)
	   integer tetra(4, *), domnod(*),  domtet(*)
...
c      nbprisme =(N-1)*N/2 + (N*(N-1)*(2*N-1))/6
...
c         xi : coordinates of the vertices
         ...
         do nd = 1, 4
  	     ...
  	     enddo
c         deca = 4*nbnodesTet
c	     call mesh_simp(deca, N, 1.D0, 5, xi)
...
c          domtet
	       ...
	       nbnodesTet = (N+1)*(N+2)*(N+3)/6
	       nbnodes =4*(N+1)*(N+2)*(N+3)/6-(3*N*N+5*N+3)
...
c       of the  plans of the meshes
	   ...
	   double precision di(N), alpha(N), Rtot, mu, h, alp
        ...
        alpha(1) = 1
	   do i = 1, N-1
	    alp = alpha(i) + alpha(i)**(1.D0 - mu)
        alpha(i+1) = min((i+1)**(1.D0/mu), alp)
	   enddo
	   h = (Rtot/alpha(N))**mu
       do i = 1, N
...
c             nm =  tetra1(k, nb)
c             do i = 1, 3
c               bk(i,k-1) = xin1(i,nm) - xin1(i, nm1)
c             enddo
c          enddo
c          res = det(bk)
c          svol(s) = svol(s)  + dabs(res)
c       enddo
c      enddo
