c     Etat de modification : fini
       subroutine ordre0(gamma1, gamma2, N, Ng,  nbtetra1,
     &                   tetra1, xin1, ww0)
	   implicit none
	   include 'simplexes_keltoum.h'
	   include 'constants_keltoum.h'

       integer  N, nbtetra1,  tetra1(4, *), Ng
       double precision xin1(3,*), ww0(10, *), gamma1, gamma2, agam
     ...
     &                  w0ref(4, 4)
	   integer nbprisme, k, nm, m, ss, nnb, kk, nbr, ni, i1, k1, nt
	   external det
...
c
      s = 5
	  do nb = (s-1)*nbtet + 1, 4*nbtet+nt
	   nm1 = tetra1(1, nb)
 	    do j = 2, 4
	     nm2 = tetra1(j, nb)
		 do i = 1, 3
	      bk(i, j-1) = xin1(i,nm2) - xin1(i, nm1)
		 enddo
	    enddo
	     absdete = det(Bk)
...
c---------------------------------------------------------------
       subroutine direct_rhs(exmp, iso, gamma1, N, Ng, nbtetra1,
     &                nbnodes1, tetra1, xin1, v_rhs)
	   implicit none
	   include 'simplexes_keltoum.h'
	   include 'constants_keltoum.h'

       integer  N, nbtetra1, tetra1(4,*), Ng, exmp, iso, nbnodes1
       double precision xin1(3,*), v_rhs(*), gamma1, gamma2, agam
       integer l, i, j, s,  nm1, nm2, nb, pow_sum,  nbtet, nt
	   external pow_sum
	   double precision BT(3,3),IBT(3,3),IBT_T(3,3), IBKT(3,3),
...
!          by inverse of F_T. The result is saved in xx
     ...
     &                         wgau, dgau)
	     do ss = 1, Ng
          delt = dgau(ss)
          delt0 = 0.D0
	      if ((dabs(gamma1-4.D0).lt.zero)) delt0 = 1.D0

	      if (dabs(delt).gt.zero) then
          if ((dabs(gamma1-4.D0).gt.zero)) delt0 = delt**(gamma1-4.D0)
            idelt = 1./delt
          else
            delt0 = 0.D0
            idelt = 0.D0
          ...
          do i1 = 1, ni
           ...
           do i = 1, nq
            ...
            call matv3(BT, xp, xxp)
 		    call fphi(s, xxp, vxp)
	        res =  rr(s,  vxp)
		    call ex_function(exmp, iso, vxp, val, valrhs)
