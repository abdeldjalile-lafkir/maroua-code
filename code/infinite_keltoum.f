...
! commande : gfortran -Wunused
     ...
     &   nrm0i, nrm0f, nrm1i, nrm1f, mu, tol, alphm, alphm1
        ...
        double precision u(nbnodes1-1),v_rhs(nbnodes1-1),
     &   rhs(nbnodes1-1), v(nbnodes1-1), svol(5)
...
!        write(*, *) 'Norme Grad de la solution     (exa): ', nrm1_exa
        ...
        call mesh(N, pform, mu, xin1, tetra1, domnod, domtet, gm,
     &                nt,nbtetra,nbtetra1,nbnodes,nbnodes1)
        call calc_pas(nbtetra1, tetra1, xin1, domtet, hmax,
     &                        form_rate,  5)
...
!        write(*, *) ' The size hk (in the bounded domain):', domtet(5000)
        write(*, *) '... done ...'
...
!--------------------------------------------
        ...
        write(*, *) '    and the right-hand-side (directly)'
         call  direct_rhs(exmp, iso, gamma1, N, Ng, nbtetra1,
     &                nbnodes1, tetra1, xin1, v_rhs)
...
!          write(*, *) reso, 4*pi*(integ_rho(6)-integ_rho(8))
     ...
     &                 nbtetra1, tetra1, xin1, grad_grad)
        write(*, *) '... done...'
...
!        stop
!!----------------------------------------
        write(*, *)  'Number of equations to solve:', nbnodes1-1
        write(*, *)  'Starting CG solver...'
        call CG(nbnodes1-1, v_rhs, v,  iter, resid,
     &                 matvec, psolve, info)
        write(*, *)  'CG solver completed with info =', info
...
c       Debug output for diagnosis
        write(*, *) 'Debug: nrm0_exa=', nrm0_exa
        write(*, *) 'Debug: nrm1_exa=', nrm1_exa
        write(*, *) 'Debug: erreurL2=', erreurL2
        write(*, *) 'Debug: errGrd=', errGrd


 	     write(*, *)  'L''erreur relative euclidienne        :',
     &                 dsqrt(reso1/reso)
        if (nrm0_exa .gt. 1.D-15) then
	      write(*, *) 'L''erreur ponderee L2 relative        :',
     &                erreurL2/nrm0_exa
        else
          write(*, *) 'L''erreur ponderee L2 relative        :',
     &                'undefined (nrm0_exa too small)'
        endif
        if (nrm1_exa .gt. 1.D-15) then
	      write(*, *) 'L''erreur ponderee H1 relative        :',
     &                errGrd/nrm1_exa
        else
          write(*, *) 'L''erreur ponderee H1 relative        :',
     &                'undefined (nrm1_exa too small)'
        endif
        if (nrm0i .gt. 1.D-15) then
 	      write(*, *) 'L''erreur ponderee L2 relative (IFEM) :',
     &                erreurL2i/nrm0i
        else
          write(*, *) 'L''erreur ponderee L2 relative (IFEM) :',
     &                'undefined (nrm0i too small)'
        endif
        if (nrm1i .gt. 1.D-15) then
 	      write(*, *) 'L''erreur ponderee H1 relative (IFEM) :',
     &                errGrdi/nrm1i
        else
          write(*, *) 'L''erreur ponderee H1 relative (IFEM) :',
     &                'undefined (nrm1i too small)'
        endif
        if (nrm0f .gt. 1.D-15) then
 	      write(*, *) 'L''erreur ponderee L2 relative (MEF)  :',
     &                erreurL2f/nrm0f
        else
          write(*, *) 'L''erreur ponderee L2 relative (MEF)  :',
     &                'undefined (nrm0f too small)'
        endif
        if (nrm1f .gt. 1.D-15) then
 	      write(*, *) 'L''erreur ponderee H1 relative (MEF)  :',
     &                errGrdf/nrm1f
        else
          write(*, *) 'L''erreur ponderee H1 relative (MEF)  :',
     &                'undefined (nrm1f too small)'
        endif
	    close(24)
       end
...
c        it makes v = alpha A u  + beta v
	    ...
	    open (unit = 25, file = 'essai.dat', status = 'unknown')
         ...
         do nb = 1, nbtetra1
          do j = 1, 4
           i = tetra1(j, nb)
	       if (i.ne.1 .and. i <= nbnodes1) then
	        res = 0.D0
            do k = 1, 4
	         s = tetra1(k, nb)
             if (s.ne.1 .and. s <= nbnodes1 .and. s >= 2) then
               j1 = max(k, j)
	           k1 = min(k, j)
	           l = (j1*(j1-1))/2 + k1
               if (l <= 10 .and. l >= 1) then
...
c----------------------------------------------------------------------
	    ...
	    integer i, j, s, k, nb,  k1, j1, l
	    double precision reso(nbnodes1), reso1(nbnodes1), res
...
c
           ...
           call ordre0(gam1, gam2,  N, Ng,  nbtetra1,
     &                   tetra1, xin1, ww0)
...
c          Etat d'avancement: fini (aucun changement)
          ...
          subroutine  grad_tetra(nbt, tetra1, grad_tet, xin1)
...
c                   tetrahedra
	   ...
	   double precision grad_tet(10),grad_r(3, 4),xin1(3,*)
	   integer i, j, s, kk, tetra1(4, *), nb1, nbt, nb
