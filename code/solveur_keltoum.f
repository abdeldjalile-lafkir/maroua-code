...
c     INFO    : an integer which gives the information about convergence
      ...
      ! Increased WORK array size to avoid segmentation fault
      double precision  WORK(10000, 7)
      ...
      LDW = 10000 ! Set to match the first dimension of WORK
      INFO = 0
      IF ( N.LT.0 ) THEN
         INFO = -1
         WRITE(*, *) 'ERROR: N < 0 (N=', N, ')'
      ELSE IF ( LDW.LT. MAX(1, N ) ) THEN
         INFO = -2
         WRITE(*, *) 'ERROR: LDW < N (LDW=', LDW, ', N=', N, ')'
         WRITE(*, *) 'WORK array is too small for this problem size'
      ELSE IF ( ITER.LE.0 ) THEN
         INFO = -3
         WRITE(*, *) 'ERROR: ITER <= 0 (ITER=', ITER, ')'
      ENDIF
      IF ( INFO.NE. 0 ) RETURN
      MAXIT = ITER
      TOL = RESID
...
c---------------------------------------------------------------------
     ...
     $                    PSOLVE, INFO )
      INTEGER           N, LDW, ITER, INFO
      double precision              RESID
      double precision              X( * ), B( * ), WORK(10000, 7)
      EXTERNAL          MATVEC, PSOLVE
      double precision  ZERO, ONE
      PARAMETER       ( ZERO = 0.0D0,ONE = 1.0D0)
...
c-------------------------------------------------------------
	...
	implicit none
      ...
      double precision   sx(1), cutlo,cuthi, hitest,sun,xmax,zero, one
...
c
  ...
  105 xmax = abs(sx(i))
      go to 115
c
c                   phase 2.  sun is small.
c                             scale to avoid destructive underflow.
c
   70 if( abs(sx(i)) .gt. cutlo ) go to 75
c
c                     common code for phases 2 and 4.
c                     in phase 4 sun is large.  scale to avoid overflow.
c
  110 if( abs(sx(i)) .le. xmax ) go to 115
         sun = one + sun * (xmax / sx(i))**2
         xmax = abs(sx(i))
         go to 200
c
  115 sun = sun + (sx(i)/xmax)**2
      go to 200
c
c
c                  prepare for phase 3.
c
   75 sun = (sun * xmax) * xmax
c
c
...
c
c                   phase 3.  sun is mid-range.  no scaling.
c
      do 95 j = ix, n
