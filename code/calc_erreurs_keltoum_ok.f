...
!       (u, v)_L2 = int(u*v)dx
     ...
     &                   prodi, prodf, res1
	  ...
	  double precision res, som, ww0(10, nbtetra1)


c  	   write(*, *) '  calcul des produits d''ordre 0...'
	   call ordre0(gamma1, gamma2,  N, Ng, nbtetra1,
     &	         tetra1, xin1, ww0)
...
!       comuting norm_L2(grad_u - grad_v)
	    implicit none
        include 'fondements_keltoum.h'
        double precision u(nbnodes1-1), v(nbnodes1-1),
...
c      Etat d'avancement: fini (aucun changement)
     ...
     &         erreurL2f, errGrdf,
     &         gamma, nrm0, nrm1
c      Use common block arrays to avoid stack overflow
c      u1 and v1 are declared in fondements_keltoum.h
       double precision u1(nbnodes1-1), v1(nbnodes1-1)
...
c       Handle uninitialized domain numbers (treat as domain 1)
        if (dom.eq.0) then
          dom = 1
        endif
        ...
        if(dom.lt.5 .and. dom.gt.0) then
	     ...
	     write(*, *) 'Problem: Invalid domain number', dom, 'for node', i
	     write(*, *) 'Valid domain numbers are 1-5'
	     v(i-1) = u(i-1)  ! Use original value as fallback
	    endif
	   enddo
       gammav = gammau - alpha
       return
